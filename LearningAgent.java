...
public class LearningAgent {
    ...
    public void InitGame() throws IOException{
        ...
        Memory_Of_Grid = new Character[Board_Size][Board_Size][(Board_Size*Board_Size / 2)+1];
        Memory_Of_Y_Moves = new int[(Board_Size*Board_Size / 2)+1];
        Memory_Of_X_Moves = new int[(Board_Size*Board_Size / 2)+1];
        PlaceMove();
    ...
    public void GetBoardState() throws IOException{
        ...
        if(g.equals("continuing") != true ){
            Is_Game_Over = true;
            System.out.println("Player Board " + Me + " = " + g);
            Game_Outcome = g;
        }
        g = Server_Channel_In.readLine();
        for(int i = 0; i < Board_Size; i++){
    ...
    public void PlaceMove(){
        ...
        Memory_Of_Y_Moves[TrainingIndex] = Best_Move_Y;
        Memory_Of_X_Moves[TrainingIndex] = Best_Move_X;
	...
	public static void main (String args[]) throws Exception {
		LearningAgent Rob = new LearningAgent();
        if(args.length == 0){
        ...
        //Rob.GetBoardState();
        Character[][][] gm = new Character[Rob.Board_Size][Rob.Board_Size][Rob.TrainingIndex+1];
        int[] mx = new int[Rob.TrainingIndex+1];
        int[] my = new int[Rob.TrainingIndex+1];
        //creates a smaller game board so it doesnt pass a huge array.
        for(int ti = 0; ti <= Rob.TrainingIndex; ti++){
            ...
            mx[ti] = Rob.Memory_Of_X_Moves[ti];
            my[ti] = Rob.Memory_Of_Y_Moves[ti];

        }
        Rob.hyp.endGame(Rob.Game_Outcome, gm, Rob.TrainingIndex+1, mx, my);//train the weights
        System.exit(1);
    }
